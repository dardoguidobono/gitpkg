...
- Use a sub folder of a repo as dependency (master branch will be used)
  ...
  ```
  https://packages.triggyr.xyz/<repo>/<subdir>
  ```

- If you want to use another branch or commit instead

  ```
  https://packages.triggyr.xyz/<repo>/<subdir>?<commit-ish>
  ```
...
- In fact, usage without sub folder is also available:
  ...
  `https://packages.triggyr.xyz/<repo>`

  `https://packages.triggyr.xyz/<repo>?<commit-ish>`
...
yarn add https://packages.triggyr.xyz/EqualMa/gitpkg-hello/packages/hello

# dep: [EqualMa/gitpkg-hello] > [packages/core] # branch=[feat/md]
yarn add https://packages.triggyr.xyz/EqualMa/gitpkg-hello/packages/core?feat/md
```

## More Formal API

```
https://packages.triggyr.xyz/api/pkg?url=<repo>/<subdir>
https://packages.triggyr.xyz/api/pkg?url=<repo>/<subdir>&commit=<commit-ish>
```

Or if you want to make the file format clear:

```
https://packages.triggyr.xyz/api/pkg.tgz?url=&commit=
```
...
- Simplest API
  ...
  ```
  https://packages.triggyr.xyz/<repo>/<subdir>?<commit-ish>&scripts.<script-name>=<script-content>
  ```

- More Formal API

  ```
  https://packages.triggyr.xyz/pkg?url=<repo>/<subdir>&scripts.<script-name>=<script-content>
  https://packages.triggyr.xyz/pkg?url=<repo>/<subdir>&commit=<commit-ish>&scripts.<script-name>=<script-content>
  ```
...
cd hello-gitpkg
npm init -y
npm install 'https://packages.triggyr.xyz/EqualMa/gitpkg-hello/packages/hello-ts?master&scripts.postinstall=npm%20install%20--ignore-scripts%20%26%26%20npm%20run%20build'
```
